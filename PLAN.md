...
(keyboard macros + keypad navigation, D-50), the Scripts-editor UX rework
(Test panel + regex validation + list-row quick wins, D-51), **Search-and-
Destroy fully live-verified** (root-caused + fixed via the harness, with
scan/consider colour parity, D-55/D-56), **local-plugin install Phase 1**
(run a plugin from a local path, in place, D-57), and plugin-compat hardening
from live testing (D-58).
...
| Personal-plugin install — local path, load in place (Phase 1) | ✅ built (unreleased, D-57) |
| Personal-plugin install — URL/network (Phase 2) | ⬜ deferred (needs approval) |
