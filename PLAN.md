...
| D-74 | 2026-05-31 | **Defer `OnPluginConnect` until the character is in-game (plugin-init experiment).** Plugins that probe the server on connect (`slist`, `cp info`, …) ran their commands during login/MOTD, where they fail. `OnPluginConnect` is now held from the raw socket connect until the first in-game `char.status` (state ≥ 3) — the same signal the D-70 broadcast gate uses — fired once per connection by whichever comes first: the in-game signal (in `dispatchGMCP`, before the first in-game `char.status` reaches plugins, so connect precedes broadcasts) or a 45s fallback timer (insurance for a stuck login / a MUD that never sends state 3). `OnPluginInstall` still runs at load (registers triggers + reads cached state — well-written plugins already state-guard their probes) and native plugins still connect on the raw connect (their connect is login-safe, e.g. AsciiMap just toggles an out-of-band telnet option). New `SessionController+PluginLifecycle.swift` (`firePluginConnectIfNeeded`/`schedulePluginConnectFallback`); `updateState`/`syncTimerLoop` split into `SessionController+ConnectionState.swift` to hold the file budget. Test: `CharStatusGateTests.deferredConnect` | adopted |
