...
**Last rewritten:** 2026-05-29 · **Latest release:** `v0.2.3` (MIT relicense +
clean copyleft-free binary, iTerm2 theme gallery, first Preferences pass, app
icon) · **HEAD:** `main` well ahead of `v0.2.3`, **preparing `v0.3.0`** (cut on
the user's green flag): UI-revamp finish, Rich Exits (D-45), Help panel
(D-46/D-52), mapper colour fix (D-47), shim wins (D-48), Phase-7 features —
Inventory Serials, Session Logging, Notifications (D-49), MacroEngine (D-50),
Scripts-editor rework (D-51), **Search-and-Destroy live-verified** (D-55/D-56),
the **empty-line/bare-Enter fix** (D-60), the **Plugin Library** — one unified
import mechanism under `~/Documents/Proteles/` (D-59 plan → D-61, Phases A/B/C;
supersedes the D-57 local-plugin framing), and **community-plugin shim
hardening** from a 12-plugin load audit (D-62).
...
| MacroEngine (keybindings/keypad navigation) | ✅ built (unreleased, D-50) |
| S&D live-verified (xcp/go/nx/consider/scan) + scan/consider colour parity | ✅ built (unreleased, D-55, D-56) |
| Empty-line / bare-Enter sent raw (bypasses alias/mapper/S&D expansion) | ✅ built (unreleased, D-60) |
| Plugin Library — unified import under `~/Documents/Proteles/` (Mac/URL), export, registry (Phases A/C) | ✅ built (unreleased, D-59/D-61) |
| Plugin Library — data relocated: global `Databases/`, per-character `data/<char>/` (Phase B) | ✅ built (unreleased, D-61) |
| Scripts under the tree — split by kind, per-kind global toggle | ✅ built (unreleased, D-61) |
| Community-plugin shim hardening (12-plugin load audit: io, telnet_options, check, SaveState, …) | ✅ built (unreleased, D-62) |
| `Accelerator`/`AcceleratorTo` → live MacroEngine keybind (utils dialog family alongside) | ✅ built (unreleased, D-63) |
| Compatibility report reworked: folder-aware, two-state, quiet (no FUD); dependency-nag stubbed | ✅ built (unreleased, D-64) |
| Plugin outbound HTTP (`async`) | ⬜ deferred post-0.3.0 (known limitation; `docs/plans/ASYNC_HTTP_PLAN.md`) |
| `v0.3.0` release cut (on the user's green flag) | ⬜ pending |
| leveldb, TTS, remaining Preferences tabs, phase-2 follow-ups | ⬜ Phase 7 (plans in docs/plans/) |
