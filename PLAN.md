...
**Last rewritten:** 2026-06-02 · **Latest release:** `v0.4.2` (tagged + GitHub
release, non-notarized build): a **live-testing reliability + UX batch** on top
of `v0.4.1`. Headline: the **full six-bar status bar** (Health/Mana/Moves/TNL/
Enemy/Alignment — always-on Enemy, per-bar toggles + colour pickers, number
modes, profile-sourced Moves colour, tier-coloured Align with boundary ticks,
outlined text, flat fills + optional quarter ticks — D-80); **surviving an
Aardwolf "ice age"** (MCCP2 copyover restart — D-81, live-confirmed); a **fix
for the session-logging crash** (App-protocol `@MainActor` isolation trap —
D-82, live-confirmed); and a **navigation/plugin-correctness batch** found by
live testing and each fixed with a faithful, fails-without-the-fix test:
`AddTriggerEx` sequence honoured so dinv's wish-capture trigger wins (worn-portal
nav — D-84), S&D `gmcp()` leaves stringified at every depth so `xcp`/`go`/`nx`
run the configured scan on arrival (D-85), custom exits preserved across a GMCP
`room.info` (no longer wiped on revisit — D-86), and a **segment-by-segment
portal walk** that waits for the portal to land before the follow-on `run`
(D-87). Plus: GMCP replay to deferred-loaded plugins (D-83), **local-time**
recording/transcript stamps (D-88), an **opt-in tag-line gag** (`{rname}`/
`{coords}`, default off — D-89), a stub-audit recorded in `KNOWN_ISSUES`, and the
vendored **Search-and-Destroy synced to its `proteles-snd-1.2` release**.
**1108 tests, four gates green.**
**Prior release `v0.4.1`:** hermetic plugin enable/disable (D-76), the dinv
`wish list` host gag (D-77/D-79), dock-drag reorder (D-78), a `GAG` transcript
category, TLS-reference cleanup.
**`v0.4.0`:** native leveling analytics — the **Levels** window over the bundled
**leveldb** plugin (D-69/D-71), **plugin outbound HTTP (`async`)** (D-67), a
plugin-reliability batch (D-70/D-72/D-73/D-74), **dinv + leveldb DB import/reset**
(D-75), input/logging polish (D-68).
**`v0.3.0`:** the **Plugin Library** under `~/Documents/Proteles/` (D-59→D-61),
**Phase-7 features** (D-49/D-50/D-51), **Rich Exits** (D-45), **Help panel**
(D-46/D-52), **Search-and-Destroy live-verified** (D-55/D-56), the
**empty-line/bare-Enter fix** (D-60), **community-plugin shim hardening** (D-62),
the **`Accelerator`→MacroEngine bridge** + `utils` dialogs (D-63), an **honest,
quiet compatibility report** (D-64), and shim closes (D-65/D-66).
...
**local-plugin install Phase 1** (run a plugin from a local path, in place —
...
- **Personal-plugin install Phase 2** — URL/network installer + consent flow
  (deferred for explicit approval; the network half of `LOCAL_PLUGIN_INSTALL_PLAN.md`).
...
| D-58 | 2026-05-28 | **Plugin-compat hardening from live testing (real community/local plugins exercising the shim).** Four fixes. (1) **Plugin directory trailing slash** — `GetInfo(60)`/`GetPluginInfo(id,20)` return the plugin dir, and MUSHclient ends it with a separator so plugins do `GetPluginInfo(id,20) .. "x_db.lua"`; our loaders passed no slash → `…/<folder><file>` "cannot open". `SessionController.directoryPath` now guarantees the slash (both the imported and personal loaders). (2) **`world` global** — MUSHclient exposes the API as fields on a global `world` object (`world.Note(...)` ≡ `Note(...)`); added a `world` proxy whose `__index` returns the matching global, so dot-form calls work instead of "attempt to index global 'world'". (3) **Honest compatibility report** — the import report's findings were stale (flagging `AddTriggerEx`/`AddAlias`/`EnableTrigger*`/`Hyperlink`/`DoAfter`/`CallPlugin`/`GetPluginInfo` as unsupported when the dinv/S&D work made them real); reframed around "does it work?": moved them to supported, dropped the call-name list (noise), downgraded miniwindows to a warning (commands run, the self-drawn panel won't show), kept genuine limits in plain words (one-shot `AddTimer`, `async`, unbundled `require`, `luacom`), and added a "loads companion files → use Add Local…" hint. (4) **S&D GMCP replay on re-attach** — a host re-created mid-session (a DB import / plugin change re-runs the world load) started blank, and Aardwolf only re-sends `char.status` on a state change, so `xcp` sat in "unknown state"; the session now caches the latest raw GMCP per package and replays it (char.base/status → room.info → rest) on attach while connected. **Open: a "mapper loses its DB" report after reload churn — not reproduced in code (schema is `CREATE IF NOT EXISTS`; re-attach reloads the graph from the same DB); awaiting a live repro + transcript (NO-GUESSING — see the diagnostic questions logged with the user).** Tests: `siblingModuleResolves`, `worldGlobalProxies`, `PluginImporterTests`, `reattachReplaysGMCPSnapshot` | adopted |
| D-59 | 2026-05-29 | **Plugin Library — one discoverable plugin home, replacing the "imported vs personal" split (`docs/plans/PLUGIN_LIBRARY_PLAN.md`; supersedes the D-57 local-plugin framing).** Reframed from user journeys (Windows migrant updating from a friend's file; GitHub/URL user; multi-file plugin; active developer who needs files an IDE can reach): there is no "personal vs imported" difference — both are just *a plugin the user added*; what differs is **where it came from** (→ how it updates). One mechanism, in a **user-visible, hand-editable** location: `~/Documents/Proteles/Plugins/<Plugin Name>/` — one self-contained dir per plugin (its `.xml` + `.lua` + a `plugin.json` manifest recording origin for the manual Update). **No "personal/private/local" wording anywhere.** Settled scoping: **code global** (shared across characters), **data per-character**; **mapper + S&D DBs are global** → `~/Documents/Proteles/Databases/` (`Aardwolf.db`, `SnDdb.db`); **dinv data per-character**; **explicit-add** (a registry, not a folder scan); **all refreshes manual**; **no migration** (single user, fresh start). One **Add Plugin…** (From your Mac = copy file/folder in; From a URL = download+extract via a generalised `PluginDownloader`, ex-`SearchAndDestroyInstaller`), per-row enable/Reveal-in-Finder/Update/Remove. **Phase A** (this deliverable): the `ProtelesPaths` home + `PluginLibrary` registry + per-plugin dir + unified loader + new Plugins UX + terminology drop — **data paths unchanged** (no broken intermediate, no sandbox-root churn). **Phase B:** relocate data into the tree (per-plugin `data/<profile>/` + sandbox roots; mapper/S&D → `Databases/`; dinv per-character) — touches the open mapper-DB area (path change only, NO-GUESSING). **Phase C:** `Scripts/`+`Aliases/` + export/share | adopted |
| D-89 | 2026-06-02 | **Opt-in gag of leftover Aardwolf tag lines (`{rname}`/`{coords}`/…).** Live finding: every `map` response leaks `{rname}<name>` + `{coords}<x,y>` to output (151× in one session) — `AsciiMap` gags the `<MAPSTART>`…`<MAPEND>` block but not the telnet-102 tag header lines before it, and no plugin on the user's Proteles set consumes them. User wanted a general `{}`-line gag, **only if provably non-destructive + non-disruptive**. Analysis confirmed both: the gag is a *display-only* decision in `appendLineThroughScripts` made AFTER `scriptEngine.process` (all scripts + generic-shim plugins), S&D, and Rich Exits, with `disposition.effects` applied regardless — so every plugin still receives + acts on the line (non-disruptive, structural, same path as the existing `richexits`/`wishprobe` gags); and the line stays in the `.jsonl` recording + `.log` transcript (a new `[tag]` reason), so nothing is destroyed. New `SessionController.isAardwolfTagLine` matches ONLY the tagged-output grammar (`^{tag}`/`^{/tag}`, lowercase identifier, no internal spaces) — Aardwolf never starts player prose/says/tells/channels with `{lowercaseword}`, so real content is never hidden; tested on the **outgoing** line so a tag a plugin transformed into shown text (Rich Exits' clickable exits) is never gagged, and dinv's `{ DINV fence N }` (space/caps) doesn't match. Wired as a **default-OFF** preference (`gagTagLines`, mirroring `Omit blank lines`: View-menu + Preferences toggle, `@AppStorage`, pushed via `setGagTagLines`). Tests: `AardwolfTagLineTests` (matches real tags; ignores prose/says/channels/mid-line; rejects spaces/caps/no-close/empty/digit-first). (Also split `SessionController`'s recording accessors into `SessionController+Recording.swift` to stay under the 600-line file budget.) | adopted |
| D-88 | 2026-06-02 | **Recording/transcript timestamps + filenames in local time (fixes a confusing UTC-vs-local split).** User report: a session started at 10:17 local produced `session-20260602-091759.log` and `…Z` line timestamps an hour "behind" — looked like a DST/clock bug. Not a bug per se: `SessionRecorder` (filename) and `SessionTranscript` (in-file lines) both stamped **UTC** (the user is BST = UTC+1, so a constant 1h offset that resembles DST). The genuine defect was an **inconsistency**: the opt-in Session Logging feature (`ProtelesApp.logTimestampFormatter`) already names its files in **local** time, so the two log paths disagreed by the UTC offset. Resolution (user picked "local everywhere"): `SessionRecorder.defaultRecordingURL` drops the UTC override (now local, `en_US_POSIX` locale for a stable format) and `SessionTranscript` uses `TimeZone.current` — `.withInternetDateTime` then emits the zone offset (`+01:00`) instead of `Z`, so lines read as the user's wall clock and stay unambiguous + sortable. All three log paths are now local. Test: `SessionTranscriptTests` computes the expected prefix with `TimeZone.current` (machine-independent) instead of asserting a fixed `…Z`. (Caution noted for future debugging: don't compare a UTC `…Z` transcript time against a `stat`/local time — that mismatch produced a phantom "the recording predates the build" earlier this session.) | adopted |
| D-87 | 2026-06-02 | **Walk a portal route segment-by-segment, waiting for the portal to land before the follow-on `run` (fixes `xcp`/`go`/`nx` portal navigation).** Live: manual `dinv portal <id>` worked, but the same hop via `xcp` aborted. From a recording, `goto` to a far room fired `enter` (dinv's portal entry, the player held the chaos portal) and `run 4swndsde5su` in the **same millisecond** (`.062`); the whoosh to the destination only landed 248ms later (`.310`), so the `run` executed from the wrong room → "Alas, you cannot go that way" → "Too many run errors. Aborting speedwalk." Root cause: `Mapper.route` emitted the entire speedwalk at once (`Speedwalk.commands(path).map(.execute)`), racing the portal command's async whoosh against the immediately-following `run`. The reference mapper waits between special steps. Fix: `Speedwalk.segments(_:)` now returns each segment with the room uid it should land in; `Mapper` keeps a pending segmented walk (`walkSegments`/`walkIndex`/`walkExpect`) and `route` sends only the **first** segment, arming the rest. On each inbound `room.info`, `SessionController` calls `Mapper.advanceWalk()`, which releases the next segment only once the previous segment's destination room has actually arrived (so a portal hop blocks the follow-on `run` until the whoosh). A portal-free path is still one `run` segment (no behaviour change). Tests: `MapperWalkTests.portalGotoWaitsForArrival` (a portal goto emits ONLY the portal command; nothing released while still in the start room; the `run` is released after the destination `room.info`) — fails (run leaks into the goto) when reverted to batch emission. (The unused step-verifying `StepWalker` is left in place; this segment walker supersedes it for goto.) **Live-verify:** worn-portal `xcp`/`go`/`nx` now complete the walk after the portal | adopted |
| D-86 | 2026-06-02 | **Preserve custom exits across a GMCP `room.info` (fixes custom exits silently vanishing on revisit).** User suspicion: the mapper import dropped custom exits. Investigation: the **import is fine** (`merge` does `INSERT OR IGNORE … exits`, bringing all rows — source had 1492 custom exits); the loss is at **live-update** time. `Mapper.ingestRoomInfo` rebuilt a room's exits from only the GMCP-reported compass dirs, and `saveExits` does `DELETE FROM exits WHERE fromuid=? ` then reinserts that set — so **every revisit wiped the room's custom exits** (`open gate;n`, `say Castle`, `enter hole`, …). 44 of the user's were already gone (the rooms they'd walked through); the rest survived (unrevisited). The reference mapper never deletes them — `aard_GMCP_mapper`'s `save_room_exits` only upserts GMCP dirs, leaving non-compass exits untouched. Fix: `ingestRoomInfo` (via a new `mergedExits(gmcp:existing:)`) carries forward every existing **non-compass** exit (`!RichExits.isCardinalDirection`) into the rebuilt set, so both the in-memory graph and the delete-then-insert keep them — matching the reference outcome. Recovery for the already-lost 44: a re-import (`INSERT OR IGNORE`) restores the missing rows without touching anything changed since. Tests: `MapperTests.revisitPreservesCustomExits` (a `room.info` revisit with a changed compass-exit set keeps the custom exit in BOTH the in-memory graph and the DB) — fails on both without the carry-forward | adopted |
| D-85 | 2026-06-01 | **Stringify GMCP leaves at every depth in the S&D host (fixes the `xcp`/`go`/`nx` post-navigation local scan).** Live report: `xcp`/`go`/`nx` navigate correctly but never run the "last bit" — the local scan at the destination (smartscan / qw / consider, selected by `xset nx`). Root cause (host bug; works in MUSHclient): S&D detects arrival in `OnPluginBroadcast` on `room.info`, setting `current_room.rmid = gmcp("room.info").num` and firing `action_on_destination_arrived()` only when `going_to_room == current_room.rmid`, where `going_to_room = tostring(room_id)` (a **string**). In MUSHclient that comparison works because the Aardwolf GMCP handler (`aard_GMCP_handler.xml`) recursively `stringify()`s **every** decoded value ("we've historically treated numbers as strings"), so `gmcp("room.info").num` is the string `"2339"`. Our S&D host's `gmcp()` binding serialised a *scalar* leaf as a quoted string (correct) but a *nested* leaf inside a table literal (`snd_lua_literal`) as a **bare Lua number** — so `gmcp("room.info").num` came back as the number `2339`, and `"2339" == 2339` is false in Lua → arrival never detected → scan never sent. Fix: `snd_lua_literal` now quotes number/boolean leaves (`string.format("%q", tostring(v))`) like the scalar path and the reference handler, so every leaf at every access path is a string. (Aardwolf sends `"num": 2339` as a JSON number — confirmed from a live transcript — so the divergence was purely our number-vs-string projection, not the wire.) Scope is the S&D runtime only; the generic shim's `gmcp()` already stringifies. Tests: `SearchAndDestroyHostTests.gmcpLeavesAreStringsAtEveryDepth` (`gmcp("room.info").num` is the string "2339"; the literal `tostring(2339) == gmcp(...).num` arrival check succeeds; nested `exits`/`coord` leaves are strings too). **Live-verify:** `xcp`/`go`/`nx` now run the configured local scan on arrival | adopted |
| D-84 | 2026-06-01 | **Honour the `AddTriggerEx` `sequence` argument for runtime triggers (fixes portal navigation when the portal is worn — "Finding 1").** Live report: `xrt`/`go`/portal hops removed the wrong item (the hold-slot Lasso) instead of the worn portal, then tried to `hold` the portal and failed ("You do not have that item"). Works in MUSHclient, so by definition a host bug. Root cause, traced from `session-20260601-183423.log`: dinv's `inv.portal.use` branches on `dbot.wish.has("Portal")`; that returned **false** because dinv's wish parse never saw the **owned** wish lines (the `*`-prefixed rows). dinv registers its wish-capture trigger via `AddTriggerEx(..., sendto.script, 0)` — **sequence 0**, so in MUSHclient it evaluates *before* any other trigger and captures every wish line exclusively. Our runtime `AddTriggerEx` path **dropped the sequence argument**: `ScriptEffect.addTrigger` carried no sequence and `addDynamicTrigger` built the `Trigger` with the default 100. At sequence 100 (tie-broken by insertion order, i.e. last), a co-loaded plugin's stop-on-match XML trigger (`keep_evaluating="n"` ⇒ `continueEvaluation == false`) that also matched the `*` lines fired first and **broke** the evaluation loop (`TriggerEngine`: `if !trigger.continueEvaluation { break }`), so dinv's wish trigger never ran on the owned lines → `has("Portal")` false → wrong slot. The dinv-only harness parsed fine (no competing plugin), confirming a live multi-plugin interaction. Fix: thread `sequence` end-to-end — Lua shim `AddTriggerEx` forwards `seq` to `proteles.addTrigger`; `ScriptEffect.addTrigger` gains a `sequence` field; `addAutomationEffect` reads arg 4 (default 100; absent for bare `AddTrigger`/`AddAlias`); both `ScriptEngine` and `SearchAndDestroyHost` `addDynamicTrigger` set `Trigger.sequence`. (`addxml.trigger` already forwarded `t.sequence` into `AddTriggerEx` — the drop was solely in the `AddTriggerEx`→effect→`Trigger` chain.) Left `continueEvaluation` at `true` for runtime triggers: sequence 0 makes dinv's capture fire first regardless, and forcing stop-on-match would reintroduce the documented `keep_evaluating` footgun. Tests: `CompatShimTests` now asserts the threaded `sequence: 50`; `ScriptEngineTests.lowSequenceTriggerWinsOverStopOnMatch` (a sequence-0 continuing trigger still fires though a sequence-100 stop-on-match trigger matches the same line). **Live-verify:** worn-portal `xrt`/portal nav now removes the worn portal, not the hold item | adopted |
| D-83 | 2026-06-01 | **Replay pre-load GMCP to deferred-loaded plugins (fixes a plugin computing the wrong effective level).** Plugins load on the first in-game `char.status` (D-74), but `char.base` — which carries `tier` + `level`, the inputs an Aardwolf plugin uses to compute the **effective level** (e.g. level 68 / tier 4 ⇒ 104) — arrives earlier, during login. An event-driven plugin (one that recomputes on the `char.base` `OnPluginBroadcast`) therefore never saw it: the broadcast fired before the plugin loaded, so it stayed at the base level and (live report) auto-selected a too-low-level spell. MUSHclient doesn't hit this — its plugins load at connect and catch every broadcast. Fix: `activatePluginsIfNeeded` now calls `replayGMCPToLoadedPlugins` right after the deferred load (before `OnPluginConnect`), re-delivering each cached package (`latestGMCPByPackage`, priority `char.base`/maxstats/worth/vitals/room.info first) via `applyGMCP` (re-fires `OnPluginBroadcast`; `proteles.gmcp` is already current) + `deliverGMCPSubnegotiation` (the option-201 path). `char.status` is excluded — the in-game one that triggered activation is delivered fresh immediately after. Mirrors the existing S&D `replayGMCPSnapshot` + dinv's char.base replay. (Confirmed not a stringification issue: dinv defines `stateActive = "3"` and compares the `gmcp()` state to it, so our scalar-stringification matches MUSHclient and was a red herring.) Tests: `DeferredPluginGMCPReplayTests` (a plugin loaded after `char.base` still sees its `tier`; non-vacuous — fails without the replay) | adopted |
| D-82 | 2026-06-01 | **Fix a crash (Swift actor-isolation trap) when session logging is enabled.** Live crash report (`EXC_BREAKPOINT`/`SIGTRAP`) on connect, with session logging on: `dispatch_assert_queue(main)` → `swift_task_checkIsolatedSwift` inside `ProtelesApp.makeLogURL`/`pruneLogs`, reached via `SessionController.startSessionLogIfEnabled` → the `logFileURL` closure. Root cause: SwiftUI's `App` protocol is `@MainActor`, so conforming `ProtelesApp` infers `@MainActor` onto **all** its members — including the static log-URL helpers. The `SessionController` **actor** invokes the `@Sendable logFileURL` closure from its own (off-main) executor on connect, so those inferred-`@MainActor` helpers ran off the main actor and the runtime isolation check trapped. (Compiled via a dynamic check; only trapped at runtime, and only with logging enabled — a latent bug since the logging feature, surfaced when the user turned logging on to capture an ice age.) Fix: mark the log-URL helpers (`makeLogURL`, `logsDirectory`, `perWorldSubfolder`, `pruneLogs`, `logFolderName`, `logContext`, `logTimestampFormatter`) **`nonisolated`** — they do pure filesystem + `UserDefaults` work and are *designed* to run off-main (the `logContext` world-name holder is already `NSLock`-guarded `@unchecked Sendable`). NOT the D-81 ice-age change (that's networking; this is app-target isolation). Live-confirmed (2026-06-02): connect-with-logging no longer crashes | adopted |
| D-81 | 2026-06-01 | **Survive an Aardwolf "ice age" (MCCP2 copyover restart).** On an ice age the server `exec()`s the new binary keeping the socket open (TCP never drops, so reconnect logic correctly does nothing), then **ends the MCCP2 compressed stream and re-negotiates compression in plaintext**. Proteles didn't recover: (1) `Inflater.inflate` mishandled `Z_STREAM_END` — when the compressed stream ended with input bytes still pending (the plaintext re-negotiation tail), the `while avail_in > 0` loop neither consumed them nor exited (`Z_STREAM_END` wasn't `Z_BUF_ERROR`), so it **spun forever** or the next call threw `Z_DATA_ERROR`; and (2) `LinePipeline.consume` force-fed *all* bytes through the inflater before telnet parsing, so post-copyover plaintext (the new `COMPRESS2`) was never seen, and a thrown inflate hit `SessionController+Inbound`'s catch → **`disconnect()`**. Fix mirrors Mudlet's `ctelnet.cpp`: `Inflater` now exposes `streamEnded` + `leftover` (the unconsumed plaintext tail) on `Z_STREAM_END` and stops; `LinePipeline` (via a new `inflateOrPassThrough` helper used at both inflate points) drops the inflater on stream-end and routes the tail + rest of the chunk through the plaintext/telnet path, where a fresh `COMPRESS2` re-activates compression. Also: the **GMCP handshake re-sends on every server `WILL GMCP`** (not once per connection) so the HUD/status modules resume after the copyover re-negotiation (handshake is idempotent; dropped the `gmcpHandshakeSent` once-guard). Tests: `InflaterTests` (stream-end exposes the tail; an open stream doesn't end), `LinePipelineMCCP2Tests.iceAgeCopyoverRecovers` (open stream → Z_FINISH end → plaintext `COMPRESS2` + fresh stream all recover, no hang/disconnect). **Live-confirmed (2026-06-02): survived a real ice age, HUD resumed** (the GMCP-re-handshake assumption — that the server re-sends `WILL GMCP` post-copyover — held) | adopted |
...
| D-57 | 2026-05-28 | **Personal-plugin install — run a plugin from a local path, in place (Phase 1 of `docs/plans/LOCAL_PLUGIN_INSTALL_PLAN.md`; superseded by D-59).** Lets a user run their own MUSHclient plugin from their own disk **without copying it into app-support**, so local plugins stay on the user's machine (the capability + UI are described generically — specific plugins are never named in repo/commits/docs, per the privacy rule). `LocalPluginStore`: a per-profile JSON store (`plugins/<id>/local-plugins.json`) of `{id, path, enabled}` references, mirroring `ScriptStore` (atomic rewrite, backward-compatible decode), kept out of `WorldProfile` so there's no Codable migration on the core type. `resolvePluginXML(at:)` accepts a `.xml` *or* a folder (resolves the `.xml` inside, preferring a name match) — the picker offers both. `SessionController.loadLocalPlugins` loads each enabled reference in place via the existing shim, with the module search path = union of the imported-plugins dir + every local plugin's own folder (so `dofile`/`require` of sibling `.lua` resolve — the dinv pattern). UI: a Plugins-window "Add Local…" action + a "Personal" section (per-world enable/remove; remove drops only the reference, never the file). **URL/network install (Phase 2) deferred for explicit approval** (the network + consent half). 6 tests; lives in `SessionController+Plugins.swift` | adopted |
