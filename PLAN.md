...
- ✅ **First-run onboarding.** The Worlds window opens automatically on first launch (UserDefaults `hasLaunchedBefore`) so a new user lands on connect / credential entry; later launches autoconnect the active profile instead.
- ✅ **Command input history + autocompletion.** `CommandHistory` (MudCore, pure/tested) backs an `NSTextField`-based input: Up/Down recall (Mudlet-style global dedup, clamped, draft-preserving), Tab/Shift-Tab cycle whole-line completions from history, Esc cancels. Communication/chat commands (`tell`, `reply`, `clan`, …) are excluded from completion. **D-18.**
