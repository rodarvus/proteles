...
| D-103 | 2026-06-10 | **Deployment floor: macOS 15 / iOS 18** (was 14 / 17). Motive: `searchFocused`, so ⌘F focuses the Scripts-window filter fields (§3.2 "⌘F finds") — unavailable on 14. Cost, accepted by the user: installed copies on macOS 14 stop seeing updates once a release ships with the raised Sparkle `minimumSystemVersion` | adopted |
