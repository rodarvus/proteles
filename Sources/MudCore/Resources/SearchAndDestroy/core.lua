...
function add_mob_area_index(db)
    DebugNote("Adding indexes to mob and area tables")
    db:execute([[
...
function set_target_from_quest(quest_target)
    if quest_target.room == "Outer Space" and quest_target.areaName == "The School of Horror" then
        quest_target.arid = "sohtwo"
    end
    quest_target.keyword = gmkw(quest_target.mob, quest_target.arid)
    change_target(
        {
            name = quest_target.mob,
            keyword = quest_target.keyword,
...
function do_cp_complete(name, line, wildcards)
    ...
    -- Delay by 0.5s to get daily bonus
    DoAfterSpecial(0.5, [[ history_end(history_type_campaign, history_status_complete) ]], 12)
    player_not_on_cp()
end
...
function build_room_targets(cp_gq, sqlr, sqla)
    ...
    for i, v in ipairs(list) do
        ...
        if results_found then
			-- Check if killed or seen in exact room, eligible for express
			local mobs_by_room_query =
			string.format(
				[[
					SELECT zone, room, kill_count, roomid
					FROM mobs
					WHERE
						mob = %s
						AND room = %s
			...
			for row in SnDdb:nrows(mobs_by_room_query) do
				for i, possibility in ipairs(possibilities) do
					if possibility.arid == row.zone then
						...
						possibility.results = possibility.results + 1
						-- Grant at least one kill if a matching room name found
						if possibility.kills < 1 then
							possibility.kills = 1
						end

						if possibility.roomid == tonumber(row.roomid) and possibility.kills < tonumber(row.kill_count) + 1 then
							-- Exact room id match, adjust up by one kill so it gets priority over just a standard room name match
							possibility.kills = tonumber(row.kill_count) + 1
							if is_express_target(possibility) then
								DebugNote("Express target found:", v.mob)
							end
						end
					end
			...
			-- Check if mob killed anywhere in the area, not eligible for express
			if not mob_found then
				local mobs_by_area_query = string.format(
						[[
							SELECT zone, roomid, kill_count
							FROM mobs
							WHERE mob = %s AND zone IN (%s)
							ORDER BY kill_count DESC;
						]],
				...
				-- DebugNote("mobs_by_area_query: ", mobs_by_area_query)
				for row in SnDdb:nrows(mobs_by_room_query) do
					for i, possibility in ipairs(possibilities) do
						if possibility.arid == row.zone then
							mob_found = true
			...
			for i, possibility in ipairs(possibilities) do
				key = string.format("%s-%s", possibility.arid, possibility.roomName)
				if filtered_possibilities[key] == nil then
					filtered_possibilities[key] = possibility
				else
					existing = filtered_possibilities[key]
					if possibility.kills > existing.kills then
						filtered_possibilities[key] = possibility
					end
				end
			end
			...
			possibilities = {}
			for k, possibility in pairs(filtered_possibilities) do
...
function print_room_links_ignored()
    ...
    for i, v in ipairs(ig) do
        ...
        if v.roomid == nil then
            ColourNote("#002460", "", link)
        else
...
function execute_in_room(rmid, func)
    ...
    execute_in_room_tbl = {i = 0, j = 0, rmid = rmid, f = fn, stat = ch_state}
    if (ri.rmid == rmid) then -- in target room
        func()
        execute_in_room_tbl = {i = 0, j = 0, arid = "", f = "", stat = 1}
    elseif (ri.rmid == nil) then --
        print("execute_in_room - nil room id")
        send_gmcp_packet("request room")
    else
...
function execute_in_room_tick()
    local ri = current_room
    local ch_state = current_character_state
    local eiat = execute_in_room_tbl
    if (ri.rmid ~= nil) and (ch_state ~= nil) then
        if (ri.rmid ~= eiat.rmid) then
            eiat.i = eiat.i + 1
            if (eiat.i > 200) then
                EnableTimer("execute_in_room_timer", false)
                xcp_clear_target(true)
                InfoNote("** execute-in-room:  abort timer - took too long to get to destination.")
                execute_in_room_tbl = {i = 0, j = 0, rmid = "", f = "", stat = 1}
            end
        else
            if (ch_state == "3") and (ch_state == eiat.stat) then
                eiat.j = eiat.j + 1
...
function search_rooms_results(results) -- Display list of 'go' links from hunt-trick, quick-where, etc.
    ...
    for i, v in ipairs(results) do
		...
		local name, text = "", ""
		if v.mz ~= "yes" then
			name = ellipsify(string.gsub(v.name, "@[a-zA-Z]", ""), 38)
			text = string.format("%3d  %-38s  %-7s ", mapper_area_index, name, string.format("(%s)", v.rmid))
			Hyperlink("go " .. mapper_area_index, text, "go to item " .. mapper_area_index, "lightblue", background, 0, 1)
		else
			name = ellipsify(string.gsub(v.name, "@[a-zA-Z]", ""), 34)
			text = string.format("%3d  %-34s  %-7s ", mapper_area_index, name, string.format("(%s)", v.rmid))
			Hyperlink("go " .. mapper_area_index, string.sub(text, 1, 4), "go to item " .. mapper_area_index, "lightblue", background, 0, 1)
			ColourTell("ghostwhite", background, "[")
			ColourTell("magenta", background, "MZ")
			ColourTell("ghostwhite", background, "]")
			Hyperlink("go " .. mapper_area_index, string.sub(text, 5), "go to item " .. mapper_area_index, "lightblue", background, 0, 1)
		end
...
function xg_create_window(name, line, wildcards)
	...
	local left = ((windowinfo.window_left+width) < GetSystemMetrics(78) and windowinfo.window_left) or 0
	local top =  ((windowinfo.window_top+height) < GetSystemMetrics(79) and windowinfo.window_top) or 0
	WindowCreate(
            win,
            left,
            top,
...
function callback_update_plugin(retval, page, status, headers, full_status, request_url)
    ...
    local pluginVer = PLUGIN_VERSION
    if prevBeta then
        pluginVer = pluginVer .. " (Beta " .. prevBeta .. ")"
    end
    if betaVersion then 
        upstream_version = upstream_version .. " (Beta " .. betaVersion .. ")"
    end
...
function onHelp(name, line, wildcards)
	...
	ColourNote("darkcyan", "", "Plugin Link    ", "antiquewhite", "", " : http://github.com/AardCrowley/Search-and-Destroy")
        Note()
    ...
    elseif str == "snd update" then
        ...
        ColourNote(
            ...
            unpack(
                {
                    helpWrap(
                        "Whenever Crowley announces a new version, this command will make it easy to update the Search & Destroy plugin. There is a hidden update command for beta versions: snd force update beta"
                    )
                }
            )
        )
	...
	elseif str == "maze" then
        ColourNote("yellow", "", "Syntax", "antiquewhite", "", ": xset maze")
        Note()
        ColourNote(
            "antiquewhite",
            "",
            unpack(
                {
                    helpWrap(
                        "Toggles current room as entrance to a maze. If mobs are found in a room with the same name as a toggled maze room, the maze room(s) will appear first in your room search results."
                    )
                }
            )
        )
    ...
    elseif str == "summary" then
		...
		ColourNote(
            "antiquewhite",
            "",
            unpack({helpWrap("xset express:  Try and go directly to mobs that are always found in the same room.")})
        )
        Note()
		ColourNote(
            "antiquewhite",
            "",
            unpack({helpWrap("xset maze:  Sets current room as the start of a maze.")})
        )
        Note()
        ColourNote(
            "antiquewhite",
...
function trigger_campaign_rewards_bonus_qp(name, line, wildcards, style)
    local qp = tonumber(wildcards.qp)
    cp_info_qp_reward = cp_info_qp_reward + qp
    set_variable("mcvar_cp_info_qp_reward", cp_info_qp_reward)
end
